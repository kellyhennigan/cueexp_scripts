...
infile = 'CSD12.mif'					# tensor or CSD file 
...
#targetStrs = ['nacc','caudate','putamen']		# can be many or none; if not defined, fibers will just be tracked from the seed ROI
targetStrs = ['nacc']		
excPath = ''
...
cutoff = '.075'						# determine FA cutoff value for terminating tracks (default is .1)
#init_cutoff = ''					# FA cutoff for initializing tracks 
...
outFileStr = '_lmax12'  # out file name suffix? Leave blank if not desired
	...
	for subject in subjects:  	# subject loop
		...
		for roi in targetStrs:
			...
			for LorR in LR:
				...
				if seedStr:
					cmd = cmd+' -seed_image '+os.path.join(roiDir,seedStr+LorR+'.nii.gz')
				if roi:
					cmd = cmd+' -include '+os.path.join(roiDir,roi+LorR+'.nii.gz')
				if excPath:
