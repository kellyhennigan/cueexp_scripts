function ctrBatchScore_kh(infoFile,numPathsToScore,multiThread)
 
% ctrBatchScore([infoFile = uigetfile],[numPathsToScore = '1000'])
%
...
batchFileName = fullfile(scrDir,[projectName,'_ctrBatchScore_',dateAndTime,'.sh']);
    ...
    thresh = [' --thresh ', num2str(numPathsToScore), ' --sort '];

%% Start mass loop here
     whos % list all variables used in function in the workspace
for ii=1:length(subs)
        sub      = dir(fullfile(batchDir,[subs{ii} '*']));
        subDir   = fullfile(batchDir,sub.name);
        fiberDir = fullfile(subDir,'fibers','conTrack');
       
       % For each roiPair find the associated .sh file. There are many .sh files (potentially)
        for kk=1:numel(roiPair)
%             fgOutName = ['scoredFG_',projectName,'_',roiPair{kk},'_top_REDO_',num2str(numPathsToScore)];
fgOutName = ['scoredFG_',roiPair{kk},'_top',num2str(numPathsToScore)];
