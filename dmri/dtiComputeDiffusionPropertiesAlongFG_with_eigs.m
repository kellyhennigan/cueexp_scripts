function [fa, md, rd, ad, cl, SuperFiber, fgClipped, cp, cs, fgResampled,eigvals] = ...
    dtiComputeDiffusionPropertiesAlongFG_with_eigs(fg, dt, roi1, roi2, numberOfNodes, dFallOff)
...
% edited by kjh to also return eigenvalue triplets for each fiber pathway
% node
...
if ~notDefined('roi1') && ~notDefined('roi2')
    ...
    % call function again to get eigenvalues
    eigvals = dtiFiberGroupPropertyWeightedAverage(fgClipped, dt, numberOfNodes, 'eigvals', dFallOff);
    ...
    % call function again to get eigenvalues
    eigvals = dtiFiberGroupPropertyWeightedAverage(fg, dt, numberOfNodes, 'eigvals', dFallOff);
