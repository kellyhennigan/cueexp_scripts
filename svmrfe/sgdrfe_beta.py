...
class SubjectData(object):
    ...
    def __init__(self, nii_path, beh_path, trs, lag=2):
        ...
        print("behavioral", self.behavioral)
        self.trs = trs
        self.lag = lag
        ...
        try:
            ...
            self.nii_shape = np.append(self.nii_shape, [1])
        self._read_behavior()
        #pdb.set_trace()
    ...
    def _read_behavior(self):
        trial_onset_idx = []
        trial_types = []
        for tr, val in enumerate(np.array([self.behavioral])):
            if val != 0:
    ...
    def get_trial(self):
        ...
        if self.nii_shape[3] > 1:
            ...
            X = np.squeeze(np.array(self.nii_proxy.dataobj))
            X = np.reshape(X, np.product(X.shape))
            y = np.array([self.behavioral])
        self.trial_index += 1
...
class ReducibleClassifierData(ClassifierData):
    ...
    def restore_coefs(self, coefs, dtype=np.float64):
        ...
        print(n_coefs, new_coefs)
        return new_coefs

    def coefs_to_niftii(self, coefs, nii_name='coefs.nii.gz', sample_img='tt29.nii'):
        """
        ...
        sample_img = nib.load(sample_img)
...
class SGDRFE(object):
    ...
    def test_accuracy(self):
        ...
        if self.test_subj is not None:
            ...
            print("No test subjects given, reporting training accuracy...")
            ytrue = self.rcd.y
    ...
    def save_nii(self, savename="my_coefs.nii"):
        """Used for saving coefs to niftii"""
        print("Saving coefs to niftii.")
        print(self.current_coefs)
        try:
            self.rcd.coefs_to_niftii(self.current_coefs, savename, sample_img=self.mask)
        except AttributeError:
