...
class SubjectData(object):
    ...
    def __init__(self, nii_path, beh_path, trs, lag):
    ...
    def get_trial(self):
        ...
        #print('tr_idx:'+str(tr_idx[0]))

        # if nii data is a 3d volume, just use that
        if len(self.nii_proxy.dataobj.shape)==3:
            X=self.nii_proxy.dataobj
        else: 
...
class ClassifierData(object):
    ...
    def __init__(self, project, niftii, behavioral, trs, test_subj, lag):
        ...
        print('trs inside ClassifierData object: '+str(trs))
        print('test_subj inside ClassifierData object: '+str(test_subj))
        ...
        #self.training_subjects_left = sample(self.training_subjects_left,len(self.training_subjects_left))
...
class ReducibleClassifierData(ClassifierData):
    ...
    def build_xy(self, method="all", mask=None, oversample=True):
    ...
    def mask_data(self, mask_name='maskGM_func.nii'):
        if self.X.ndim==1:
            ### ONLY IN TESTING
    ...
    def coefs_to_niftii(self, coefs, nii_name='coefs.nii.gz'):
        ...
        sample_img = nib.load('maskGM_func.nii')
...
class SGDRFE(object):
    ...
    def __init__(self, project, niftii, behav, trs, lag, test_subj=None,
                 record=None, clftype="sgd", cut=.05, C=1.0, stop_threshold=.025,
                 mask='maskGM_func.nii', descriptor='', l1_ratio=.15, max_iter=1000):
        self.project = project
        self.niftii = niftii
        self.behav = behav
        self.trs = trs
        ...
        self.rcd = ReducibleClassifierData(
            project, niftii, behav, trs, test_subj, lag, cut)
    ...
    def test_accuracy(self):
        ...
        return accuracy, metrics, predictions, train_accuracy
    ...
    def train_batch(self):
        ...
        accuracy, metrics, predictions, train_accuracy = self.test_accuracy()
        ...
        # test_subject, clftype, n_features, %features, accuracy, test_predictions, f1, precision, recall, roc_auc, r2, train_time
