...
% NOTE: put patients before controls for
plotGroups = {'patients'};
        ...
        for g=1:numel(groups)
            ...
            for c = 1:numel(stims)
                ...
                if strfind(stims{c},'-')
                    ...
                    [tc1,subjects{g,c}]=loadRoiTimeCourses(fullfile(inDir,[stim1 '.csv']),groups{g},1:nTRs);
                    [tc2,~]=loadRoiTimeCourses(fullfile(inDir,[stim2 '.csv']),groups{g},1:nTRs);
                    tc{g,c}=tc1-tc2;
                else
                    stimFile = fullfile(inDir,[stims{c} '.csv']);
                    [tc{g,c},subjects{g,c}]=loadRoiTimeCourses(stimFile,groups{g},1:nTRs);
                end
