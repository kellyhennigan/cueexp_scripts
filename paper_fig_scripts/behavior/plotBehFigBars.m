function [fig,leg] = plotBehFigBars(d,dName,condNames,groupNames,cols,...
...
if notDefined('fontSize')
    fontSize = 'default'; 
end
fontSize

if notDefined('saveFormat')
    saveFormat = '-dpng';
end

if notDefined('figWidth')
    figWidth = 'default';
end
...
%%%%%%%%%%% if plotting data for 2 or more groups: 
else
...
% title
title(titleStr,'fontName',fontName,'fontSize',titleFSize)

% ylim 
if  ~strcmp(YL,'default')
    ylim(YL)
end

if strcmpi(dName(1:4),'want') && max(d{1}(:))<=3
    set(gca,'YTick',[-3:2:3])
...
if savePath
    ...
    if ~strcmp(figWidth,'default')
        fig.PaperUnits = 'inches';
        figPos=fig.PaperPosition;
        % figWidth=3.5;
        figHeight=figWidth.*(figPos(4)./figPos(3)); % get fig height to keep same proportions
        fig.PaperPosition = [0 0 figWidth figHeight];
    end
    
    print(fig,saveFormat,'-r300',savePath)
