...
def correctMotion(inFile,r):
	...
	cmd = ('1d_tool.py -infile '+mc_str+'.1D[1..6] -censor_motion 0.5 motion'+str(r)+' '
...
if __name__ == '__main__':
	...
	for subject in subjects:
		...
		funcFileAnts = normalizeANTSFunc('pp_mid_trunc+orig')
		normalizeANTSFunc(ref_file)
		
		#funcFileAfni = normalizeAfniFunc('pp_mid_trunc+tlrc')
		#normalizeAfniFunc(ref_file)
				
			
		# #########  extract csf and wm time series
		extractCsfWmTS(funcFileAnts,'_trunc')
		#extractCsfWmTS(funcFileAfni,'_afni')

		
		# concatenate run files
		cmd = ('cat vr1.1D vr2.1D >> vr.1D')
		print cmd+'\n'
		if not justPrint:
	 		os.system(cmd)
	
		cmd = ('cat motion1_censor.1D motion2_censor.1D >> motion_censor.1D')
		print cmd+'\n'
		if not justPrint:
	 		os.system(cmd)
	
	
		# w/LAST VOLS TRUNCATED
