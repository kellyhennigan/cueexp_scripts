...
% http://www.mccauslandcenter.sc.edu/CRNL/sw/qa/nii_qa_moco.m.txt 0: check
% inputs
...
if ~exist('imgName','var') %no image specified
 imgName = spm_select(inf,'^.*\.(gz|voi|img|nii)$','Select images to average');
end;
...
fprintf('Most extreme Z-score %g (volume number %d)\n',mx, mxi);
[mx, mxi] = max(fd1d);
fprintf('Most extreme Framewise Displacement %g mm (volume number %d)\n',mx, mxi);
[mx, mxi] = max(dvars1d);
fprintf('Most extreme Framewise Variability %g (volume number %d)\n',mx, mxi);
diary off
%end reportText()
