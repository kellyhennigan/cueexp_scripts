...
for i=1:numel(subjects) % subject loop
    ...
    if ~isempty(nuisance_regfiles)
        ...
        X = [ones(size(func.data,4),1),nregs];
    ...
    for j = 1:numel(stims)
        ...
        if saveOutSingleSubjectVols
            ...
            ni = createNewNii(func,reshape(thisZ,dim(1),dim(2),dim(3)),fullfile(outDir,[subject '_' stims{j}]));
...
%   groups=groupNames
%   nii=mask
%   outPath = 'drugs.nii.gz'
% out = glm_fmri_ttest3d(Z{j}(gi==0,:),Z{j}(gi==1,:),groups,nii,outPath,'z')
% 
...
for j=1:numel(stims)
    ...
    [~,~,~,stats1] = ttest(Z{j}(gi==0,:)); % paired ttest group gi=0
    df1 = mode(stats1.df);
    [~,~,~,stats2] = ttest(Z{j}(gi==1,:)); % " " for group gi=1
    df2 = mode(stats2.df);
    [~,~,~,stats3] = ttest2(Z{j}(gi==1,:),Z{j}(gi==0,:)); % unpaired ttest
    df3 = mode(stats3.df);
